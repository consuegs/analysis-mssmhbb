...
int AnalysisWorkspaceSR4_OldMatchS()
{

	std::ofstream textout("figs/AnalysisWorkspaceSR4_OldMatchS.txt");
	...
	for (unsigned int mass = 0; mass < srmasses.size(); mass++)
	{
		...
		xframe->Draw();
		c1->Update();
		c1->Print("figs_OldMatchS/sig_SR4_" + Tsrmasses[mass] + ".png");
		c1->Print("figs_OldMatchS/sig_SR4_" + Tsrmasses[mass] + ".pdf");
		delete c1;
		...
		//Output file
		TFile *fOut = new TFile("input_2018_FH_OldMatchS/signal_workspace_" + Tsrmasses[mass] + "_SR4.root", "RECREATE");
