...
int AnalysisWorkspaceSR1_OldMatchS()
{

	std::ofstream textout("figs/AnalysisWorkspaceSR1_OldMatchS.txt");
	...
	for (unsigned int mass = 0; mass < srmasses.size(); mass++)
	{
		...
		xframe->Draw();
		c1->Update();
		c1->Print("figs_OldMatchS/sig_SR1_" + Tsrmasses[mass] + ".png");
		c1->Print("figs_OldMatchS/sig_SR1_" + Tsrmasses[mass] + ".pdf");
		delete c1;
		...
		RooRealVar norm("norm", "norm", norm_centralValue, 0.01 *norm_centralValue, 100 *norm_centralValue);
		RooRealVar ext("ext", "ext", ext_centralValue, 0.01 *ext_centralValue, 100 *ext_centralValue);
		RooArgList varsTF(mbb, x0, k, norm, ext);
		RooGenericPdf TF("TF", "TF", "norm*erf(k*(mbb-x0))*(1-ext*mbb)", varsTF);	
		...
		//Output file
		TFile *fOut = new TFile("input_2018_FH_OldMatchS/signal_workspace_" + Tsrmasses[mass] + "_SR1.root", "RECREATE");
