...
int AnalysisWorkspaceSR4_parametrized()
{
	...
	TString Tfunc[4] = {"bukin","bukin","bukin","bukin"}; // SR4
	...
	for (unsigned int mass = 0; mass < srmasses.size(); mass++)
	{
	  ...
	  cout << "mass " << srmasses[mass];
		...
		///
		/// PGET DATA_OBS HISTS FOR CR/SR (CR FROM ANALYSIS MACRO, SR FOR NOW FROM TOYS)
		///
		...
		TFile *f_bgfit = new TFile(dir+"/Inputs_Paul/FinalVersionAnalysis2017FH/BGparam/500to2000/workspace/FitContainer_workspace.root", "READ");	//SR4
		...
		xframe->Draw();
		c1->Update();
		c1->Print("sig_SR4_"+Tsrmasses[mass]+".png");
		delete c1;

		///
		/// TRANSFER FACTOR PDF(S)	
		...
		//Output file
		TFile *fOut = new TFile("input_parametrized/ReReco_signal_M-" + Tsrmasses[mass] + "/workspace/SR4/signal_workspace.root", "RECREATE");
