...
int AnalysisWorkspaceSR4_parametrized()
{
	...
	for (unsigned int mass = 0; mass < srmasses.size(); mass++)
	{
		...
		//Output file
		TFile *fOut = new TFile(("input_parametrized/ReReco_signal_M-" + srmasses[mass] + "/workspace/SR4/signal_workspace.root").c_str(), "RECREATE");
