...
int AnalysisWorkspaceSR1_parametrized()
{
	...
	for (unsigned int mass = 0; mass < srmasses.size(); mass++)
	{
		...
		normSignal = h_signal_in->Integral();
		...
		TH1F *h_JERUp_in = (TH1F*) f_JERUp_in->Get("m12_" + Tsrmasses[mass] + "GeV_SR1_1GeVbinning");
		...
		TH1F *h_JERDown_in = (TH1F*) f_JERDown_in->Get("m12_" + Tsrmasses[mass] + "GeV_SR1_1GeVbinning");
		...
		TH1F *h_JESUp_in = (TH1F*) f_JESUp_in->Get("m12_" + Tsrmasses[mass] + "GeV_SR1_1GeVbinning");
		...
		TH1F *h_JESDown_in = (TH1F*) f_JESDown_in->Get("m12_" + Tsrmasses[mass] + "GeV_SR1_1GeVbinning");
		...
		TFile *f_bgfit = new TFile("../Inputs_Paul/FinalVersionAnalysis2017FH/BGparam/200to500/workspace/FitContainer_workspace.root", "READ");
		...
		TFile *f_signal_in_unbinned = new TFile(("input_parametrized/ReReco_signal_M-" + srmasses[mass] + "/workspace/FitContainer_workspace.root").c_str(), "READ");
		...
		double getPeakStart = std::stof(srmasses[mass]);
		if (getPeakStart > 400)
		{
			mean_sigmaL = 90.;
			mean_tail_sigma = 30.;
		...
		RooRealVar mean("mean", "mean", 300, 280, 500, "GeV");
		RooRealVar sigmaL("sigmaL", "sigmaL", mean_sigmaL, 20, 90, "GeV");
		RooRealVar sigmaR("sigmaR", "sigmaR", mean_sigmaR, 20, 45, "GeV");
		RooRealVar tail_shift("tail_shift", "tail_shift", 300 * 1.2, 340, 600);
		RooRealVar tail_sigma("tail_sigma", "tail_sigma", mean_tail_sigma, 50, 150);

		TFile *f_signal_in_unbinned_bias = new TFile(("input_parametrized/ReReco_signal_M-" + srmasses[mass] + "/workspace/FitContainer_workspace.root").c_str(), "READ");
		RooWorkspace *w_signalfit_bias = (RooWorkspace*) f_signal_in_unbinned_bias->Get("workspace");
		RooAbsPdf *signal_bias = w_signalfit_bias->pdf("signal");
		signal_bias->SetName("signal_bias");
		RooRealVar signal_bias_norm("signal_bias_norm", "signal_bias_norm", normSignal);
		...
		//Output file
		TFile *fOut = new TFile(("input_parametrized/ReReco_signal_M-" + srmasses[mass] + "/workspace/SR1/signal_workspace.root").c_str(), "RECREATE");
		...
		cout << "Checking one Parameter: " << signal->getVal(mean) << endl;

		wspace.import(background_norm);
		wspace.import(signal_norm);
		wspace.import(signal_bias_norm);
		wspace.import(TF);
